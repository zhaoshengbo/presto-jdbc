...
public class ClientTypeSignature {
    ...
    public ClientTypeSignature(TypeSignature typeSignature) {
        this(
                ...
                transform(typeSignature));
    }


    public static List<ClientTypeSignatureParameter> transform(TypeSignature typeSignature) {
        List<ClientTypeSignatureParameter> parameterList = new ArrayList<ClientTypeSignatureParameter>();
        List<TypeSignatureParameter> parameters = typeSignature.getParameters();
        for (TypeSignatureParameter parameter : parameters) {
            parameterList.add(new ClientTypeSignatureParameter(parameter));
        }
    ...
    public ClientTypeSignature(String rawType, List<ClientTypeSignatureParameter> arguments) {
        this(rawType, ImmutableList.<ClientTypeSignature>of(), ImmutableList.of(), arguments);
    }
    ...
    public ClientTypeSignature(
        ...
        if (arguments != null) {
            this.arguments = unmodifiableList(new ArrayList<ClientTypeSignatureParameter>(arguments));
        } else {
    ...
    private static TypeSignature toTypeSignature(ClientTypeSignature signature) {
        ...
        List<TypeSignatureParameter> parameterList = new ArrayList<TypeSignatureParameter>();
        for(ClientTypeSignatureParameter argument : arguments){
            parameterList.add(argument.getTypeSignature().legacyClientTypeSignatureParameterToTypeSignatureParameter(argument));
        }
        return new TypeSignature(signature.getRawType(), parameterList);
    }
    ...
    public List<ClientTypeSignature> getTypeArguments() {
        List<ClientTypeSignature> result = new ArrayList<ClientTypeSignature>();
        for (ClientTypeSignatureParameter argument : arguments) {
    ...
    private String rowToString() {
        ...
        for (ClientTypeSignatureParameter parameter : arguments) {
            NamedTypeSignature namedTypeSignature = parameter.getNamedTypeSignature();
            fields.append(format("%s %s", namedTypeSignature.getName(), namedTypeSignature.getTypeSignature().toString()));
            fields.append(",");
        }

        return format("row(%s)", fields.substring(0, fields.length() - 1));
    }
