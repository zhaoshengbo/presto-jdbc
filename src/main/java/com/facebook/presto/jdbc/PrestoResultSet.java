...
public class PrestoResultSet
    ...
    PrestoResultSet(StatementClient client, long maxRows, ProgressMonitor progressMonitor)
            throws SQLException {
        ...
        List<Column> columns = getColumns(client, progressMonitor);
        ...
        this.results = flatten(new ResultsPageIterator(client, progressMonitor), maxRows);
    }
    ...
    private static List<Column> getColumns(StatementClient client, ProgressMonitor progressMonitor)
            throws SQLException {
        while (client.isValid()) {
            QueryResults results = client.current();

            if (progressMonitor != null) {
                progressMonitor.accept(QueryStats.create(results.getId(), results.getStats()));
            }
    ...
    private static <T> Iterator<T> flatten(Iterator<Iterable<T>> iterator, long maxRows) {
        MyTransformedIterator<Iterable<T>, Iterator<T>> myTransformedIterator = new MyTransformedIterator<Iterable<T>, Iterator<T>>(iterator) {

            @Override
            Iterator<T> transform(Iterable<T> from) {
                return from.iterator();
            }
        };
        Iterator<T> rowsIterator = concat(myTransformedIterator);
        return (maxRows > 0) ? new LengthLimitedIterator<T>(rowsIterator, maxRows) : rowsIterator;
    }


    private abstract static class MyTransformedIterator<F, T> implements Iterator<T> {
        final Iterator<? extends F> backingIterator;

        MyTransformedIterator(Iterator<? extends F> backingIterator) {
    ...
    private static class ResultsPageIterator
        ...
        private ResultsPageIterator(StatementClient client, ProgressMonitor progressMonitor) {
            this.client = requireNonNull(client, "client is null");
        }
