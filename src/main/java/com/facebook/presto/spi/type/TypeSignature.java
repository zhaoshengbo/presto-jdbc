...
public class TypeSignature {
    private String base;
    private List<TypeSignatureParameter> parameters;
    ...
    public TypeSignature(String base, List<TypeSignatureParameter> parameters) {
        ...
        this.parameters = unmodifiableList(new ArrayList<TypeSignatureParameter>(parameters));
    ...
    public List<TypeSignature> getTypeParametersAsTypeSignatures() {
        List<TypeSignature> result = new ArrayList<TypeSignature>();
    ...
    public static TypeSignature parseTypeSignature(String signature, Set<String> literalCalculationParameters) {
        ...
        List<TypeSignatureParameter> parameters = new ArrayList<TypeSignatureParameter>();
    ...
    private String rowToString() {
        ...
        if (!allMath) {
            throw new AssertionError(format("Incorrect parameters for row type %s", parameters));
        }

        StringBuilder fileds = new StringBuilder();
        for (TypeSignatureParameter parameter : parameters) {
            NamedTypeSignature namedTypeSignature = parameter.getNamedTypeSignature();
            fileds.append(format("%s %s", namedTypeSignature.getName(), namedTypeSignature.getTypeSignature().toString()));
            fileds.append(",");
        }
        return format("row(%s)", fileds.substring(0, fileds.length() - 1));
    }
