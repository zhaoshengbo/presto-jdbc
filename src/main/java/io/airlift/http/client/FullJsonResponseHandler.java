...
public class FullJsonResponseHandler<T> implements ResponseHandler<JsonResponse<T>> {
    ...
    @Override
    public JsonResponse<T> handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
        byte[] bytes = readResponseBytes(response);
        String contentType = response.getFirstHeader(CONTENT_TYPE).getValue();
        StatusLine statusLine = response.getStatusLine();
        Header[] allHeaders = response.getAllHeaders();
        LinkedListMultimap<HeaderName, String> multimap = LinkedListMultimap.create();
        for (Header header : allHeaders) {
            multimap.put(HeaderName.of(header.getName()), header.getValue());
        }
        if ((contentType == null) || !MediaType.parse(contentType).is(MEDIA_TYPE_JSON)) {
            return new JsonResponse<T>(statusLine.getStatusCode(), statusLine.getReasonPhrase(), multimap, bytes);
        }
        return new JsonResponse<T>(statusLine.getStatusCode(), statusLine.getReasonPhrase(), multimap, jsonCodec, bytes);
    }


    private static byte[] readResponseBytes(HttpResponse response) {
        try {
